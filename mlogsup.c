...
{
   MLogIndex *ix=NULL;
   int i,ol=sizeof(MLogIndex)+((mindex->max-1)*sizeof(MLogHeader*));
   int nl=sizeof(MLogIndex)+(((mindex->max*2)-1)*sizeof(MLogHeader*));

   ix=(MLogIndex*)malloc(nl);
   memcpy(ix,mindex,ol);
   ix->max=mindex->max*2;
   for (i=mindex->max; i<ix->max; i++) ix->entry.nextFree[i]=i+1;
   ix->entry.nextFree[ix->max-1]=IDXS+2;
   ix->entry.nextFree[ix->free]=mindex->max;
   free(mindex);
   mindex=ix;
   return mindex->free;
}
...
int makeMLog()
{
   ...
   mindex->free=mindex->entry.nextFree[idx];
   mindex->entry.hdr[idx]=ah;
...
void *addToMLog(int idx, void *area)
{
   ...
   ah=mindex->entry.hdr[idx];
   if (ah->cur>=ah->max)
      ah=mindex->entry.hdr[idx]=growMLogHeader(ah);
...
void freeMLog(int idx)
{
   ...
   mindex->entry.nextFree[idx]=mindex->free;
   mindex->free=idx;
   freeMLogHeader(ah);
...
void freeMLogAll()
{
   ...
   for (i=0; i<IDXS; i++) {
 //     if (debug) printf("--- trying %d - %d\n",i,mindex->entry.nextFree[i]);
      if (mindex->entry.nextFree[i]>IDXS+2) freeMLog(i);
   }
